...
func TestGetData(t *testing.T) {
	var serviceName, serviceOperation string
	...
	/*
		...
		server, err := NewMockTestServer(cli.log, endpoints, true)
	*/
	basicAuth := map[string]string{
		"username": servicePrincipal,
		"password": principalSecret,
	}
	server, err := NewMockTestServer(cli.log, nil, basicAuth, true)
	if err != nil {
	...
	expectedItemCount := 2
	items, err := cli.GetData(serviceName, serviceOperation, nil)
	if err != nil {
		t.Fatalf("expected success, but failed: %s", err)
	}
	if len(items) != expectedItemCount {
		t.Fatalf(
			"%s/%s response item count mismatch: %d (expected) vs. %d (received)",
			serviceName, serviceOperation, expectedItemCount, len(items),
		)
	}
